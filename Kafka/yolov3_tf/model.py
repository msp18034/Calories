...
from Kafka.yolov3_tf.layer_utils import conv2d, darknet53_body, yolo_block, upsample_layer


class yolov3(object):

    def __init__(self, use_focal_loss=False, batch_norm_decay=0.999, weight_decay=5e-4, use_static_shape=True):

        self.anchor_path = "/home/hduser/Calories/dataset/yolo_anchors.txt"
        self.class_name_path = "/home/hduser/Calories/dataset/coco.names"
        self.restore_path ="/home/hduser/model_weights/yolov3.ckpt"
        self.anchors = _parse_anchors(self.anchor_path)
    ...
    def _parse_anchors(path):
        # returned data: shape [N, 2], dtype float32
        anchors = np.reshape(np.asarray(open(path, 'r').read().split(','), np.float32), [-1, 2])
        return anchors
